...
function UserStrip(props) {
	const { obj = {}, brain, manageMode, isChatSetup, galleryMode, isSelected, isInvitations, superMan, chatObj = {}, stripMenu, setStripMenu, isSearch, isChatMember, isChatRequest, menuView, rightButtons, chatType, isNewUser, getPunishmentStatus } = props,
		[status, setStatus] = useState({ score: obj.score, mark: obj.mark, blocked: obj.blocked, linked: obj.linked, trusts: obj.trusts, reported: null, unavail: obj.unavail }),
	...
	return (
		<user-strip
			...
			class={`${modes.profile ? 'shaTop marBotL borderTop ' : isSelected && isSearch ? ' bInsetBlue  shaBot posRel borTop8' : isSelected ? 'boRadXxs posRel Halo' : isChatMember ? 'borderLight' : ''} boRadXxs  bHover ${modes.protocol || modes.menu ? 'marBotS  sideBors boRadXxs  posRel' : obj.flag === 'del' ? 'borderRed' : 'borBotLight'} flexCol posRel  bHover pointer shaComment  bBor shaBot   aliStart ${isInvitations ? '' : 'mw68'}`}>
			{/* TOP PART ------------------------------------------------------ */}
			<strip-body class={`${galleryMode === 'links' && status.linked === false ? 'borderRed boRadXs fPadHorXxxs xBold' : galleryMode === 'requests' && obj.linked === true ? 'bInsetGreen' : ''}   flexRow marAuto h100 w100 aliCen padVerXxxs justCen`}>
				<img-wrapper class="posRel marRigXs">
					<img className={`${modes.menu ? 'bsContentGlow bDarkBlue' : ''} w18  mw10  zin100 miw7 aspect169  shaBot boRadXxs`} src={obj.imgVers ? `${import.meta.env.VITE_BACK_END}/public/users/${Math.floor(Math.random() * 30) + 1}_${obj.imgVers}S.webp` : '/icons/placeholdergood.png'} alt="" />
					{!isChatSetup && !isChatMember && !isSearch && role && role !== 'member' && <span className={`${role === 'VIP' ? 'bPurple ' : role === 'guard' ? 'bgTransXs  blueGlass' : 'tGreen bgTransXs blueGlass'} boRadXxs posAbs botRight tWhite marTopXs padVerXxxs padHorXxs lh1 ${role !== 'guard' ? 'fs8 boldM' : 'fs8'}`}>{role.slice(0, 3)}</span>}
				</img-wrapper>
				<middle-section class="h100 flexCol gapXxxs justCen padTopXxs	 h100">
					...
					<title-aftertitle class={'textLeft'}>
						...
						<ContentIndis menuView={menuView} status={status} isChats={isChatSetup || isChatMember} getPunishmentStatus={getPunishmentStatus} manageMode={manageMode} galleryMode={galleryMode} isSearch={isSearch} thisIs="user" isCardOrStrip={true} modes={modes} brain={brain} obj={obj} isNewUser={isNewUser} />
					...
					<button
						...
						className={`${
							modes.roles
								...
								: `${
										manageMode === 'punish' ? (punish ? 'bDarkRed tWhite' : 'tRed xBold fs7') : manageMode === 'manage' && brain.chatSetupData?.id ? 'tRed bgTrans borderLight fs8 xBold' : obj.role === 'VIP' ? 'bDarkPurple tWhite ' : obj.role === 'guard' ? 'bDarkBlue tWhite ' : obj.role === 'admin' ? 'bDarkGreen tWhite ' : obj.role === 'spect' ? 'bRed tWhite ' : obj.role === 'member' || (!obj.role && isChatSetup) ? 'bBlue tWhite ' : 'borderRight borTop2  noBackground'
			...
			<bottom-part onClick={e => e.stopPropagation()} class="w100">
							...
							{(() => {
								...
								return Object.keys(roleMap)
									...
									.map(button => (
										<button
											key={button}
											className={`fs7 bInsetBlueTop posRel bHover ${role && button === role ? 'boldM tWhite' : ''} ${button === 'VIP' && role === 'VIP' ? 'bDarkPurple' : button === 'guard' && role === 'guard' ? 'bDarkBlue' : button === 'admin' && role === 'admin' ? 'bDarkGreen' : button === 'spect' && role === 'spect' ? 'bRed' : button === 'member' && role === 'member' ? 'bBlue' : ''}`}
				...
				{/* MENU COMPONENTS ----------------------------------------------- */}
				{!isChatSetup && !isInvitations && obj.id !== brain.user.id && <UserActionsBs obj={obj} brain={brain} status={status} setStatus={setStatus} modes={modes} setModes={setModes} isPast={false} galleryMode={galleryMode} nowAt={''} superMan={superMan} UserCardComp={UserCard} isStrip={true} />}

				{isChatSetup && modes.protocol === 'punish' && <SimpleProtocol setModes={setModes} superMan={superMan} obj={obj} target={obj.id} modes={modes} thisIs={'user'} brain={brain} setStatus={setStatus} chatObj={chatObj} chatID={chatObj?.id} />}
			</bottom-part>
		</user-strip>
	);
}
...
function areEqual(prev, next) {
	...
	return prev.obj === next.obj && prev.isSelected === next.isSelected && prev.isOpened === next.isOpened && prev.stripMenu === next.stripMenu && prev.manageMode === next.manageMode && prev.chatType === next.chatType && prev.obj.role === next.obj.role && prev.obj.punish === next.obj.punish && prev.obj.flag === next.obj.flag && prev.brain.chatSetupData?.type === next.brain.chatSetupData?.type && prev.brain.chatSetupData?.members.length === next.brain.chatSetupData?.members.length;
}

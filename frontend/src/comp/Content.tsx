...
function Content(props) {
	...
	function setCardsToContent(data, cols) {
		const isRef = !data;
		((data ??= content), (cols ??= Math.min(Math.floor(contentRef.current.offsetWidth / 360), brain.user.cols[isPastUsers ? 'pastUsers' : contView], data?.length || 1, nowAt === 'event' ? 4 : Infinity)));
		if (cols && !data) return;
	...
	return (
		<content-comp id="content" class={`fadingIn ${fadedIn.includes('Content') ? 'fadedIn' : ''} ${nowAt !== 'event' ? 'mihvh140' : ''} fPadHorXxs posRel w100 posRel block marAuto`} ref={contentRef}>
			...
			{/* MASONRY GRID --- */}
			{!snap.fetch && content?.length && numOfCols && <Masonry key={`${window.location.pathname}`} superMan={contentMan} content={content?.slice(0, nowAt !== 'event' && content?.length % 20 !== 0 && contQueue.current.length ? content.length - (content.length % 20) : nowAt !== 'home' && !showMore ? (isPast ? 20 : 4) : undefined)} config={{ contType: contView, numOfCols }} nowAt={nowAt} snap={snap} isMobile={isMobile} brain={brain} Comp={Comp} cardProps={cardProps} />}

			{/* PAGINATION BUTTONS --- */}
			{nowAt === 'event' && (
				<buttons-div class="flexCen w80  bw50 posRel bmw30 marAuto moveUp bPadXxs zinMaXl">
					{showMore && content?.length > (isPast ? 20 : 4) && (
						<button onClick={() => (setShowMore(false), setTimeout(() => window.scroll({ top: contentRef.current.offsetTop, behavior: 'smooth' }), 100))} className="mw40 bDarkBlue borRed tWhite bHover xBold fs7 boRadXs">
							Sbalit
						</button>
					)}
					{(contQueue.current.length || (!showMore && content?.length > (isPast ? 20 : 4))) && (
						<button onClick={() => (!showMore && content.length > (isPast ? 20 : 4) ? setShowMore(true) : (contentMan(true), !showMore && setShowMore(true)))} className="bInsetBlueTopXs   bBor posRel maskLowXs  shaBot bHover  tDarkBlue xBold fs14 boRadXxxs">
							Další
						</button>
					)}
				</buttons-div>
			...
			{nowAt === 'home' && <ContentEndBanner infinityTrigger={infinityTrigger} content={content} contQueue={contQueue} disableInfinite={disableInfinite} firstBatchReady={firstBatchReady} snap={snap} contentMan={contentMan} />}
		</content-comp>
	);
}
...
function propsEqual(prev, next) {
	return prev.eveInter === next.eveInter && prev.snap.fetch === next.snap.fetch && prev.map === next.map && prev.nowAt === next.nowAt && prev.fadedIn === next.fadedIn && prev.disableResize === next.disableResize && prev.showAllThumbs === next.showAllThumbs && prev.isMobile === next.isMobile && prev.brain?.user?.settings?.showAllThumbs === next.brain?.user?.settings?.showAllThumbs;
}

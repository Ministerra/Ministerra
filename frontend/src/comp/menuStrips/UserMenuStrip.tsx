...
const UserMenuStrip = props => {
	const { obj = {}, chatObj = {}, modes = {}, setStatus = () => {}, isCardOrStrip = false, galleryMode = '', isChatMember = false, isSearch = false, brain = {}, status = {}, setModes = () => {}, nowAt = '', isEventPreview = false, isPast = false, removeAlert = null, superMan = null } = props;
	const { id } = obj,
	...
	const src = Object.fromEntries(
		Object.entries({
			...
			profil: !obj.blocked && !isPast && isCardOrStrip ? async () => showUsersProfile({ obj, brain, chatObj, setModes: n => setModes({ ...n(), profile: n().profile || null, menu: true }), modes, setStatus }) : null,
			...
			tresty: chatObj.type !== 'private' && !isSearch && !galleryMode && obj.role === 'member' && hasRole && obj.id !== brain.user.id ? () => setMode('protocol', modes.protocol === 'punish' ? null : 'punish') : null,
		...
		<user-menu class={`zin3000 point posRel  w100`} onClick={e => e.stopPropagation()}>
			...
			{modes.trust && (
				<confirm-box onClick={e => e.stopPropagation()} class="flexCol textAli padVerXs padHorS borTopLight shaComment bgWhite">
					...
					<span className="fs7 inlineBlock marBotS">Důvěrní uživatelé představují izolovaný seznam tvých spojenců s nimiž máš velmi vřelý vztah a chceš je mít možnost jednoduše zacílit svým obsahem (událostmi, komentáři apod.) a nebo dle potřeby omezit viditelnost (soukromí) tvého obsahu pouze na ně. Důvěrníci vidí totéž co tvoji spojenci (opačně to platit nemusí)</span>
			...
			{modes.profile && <UserCard obj={{ ...modes.profile, ...(obj.first ? obj : chatObj.members.find(m => m.id === obj.user)) }} cardsView={brain.user.cardsView.users} isProfile={true} galleryMode={galleryMode} brain={brain} setModes={setModes} />}
			...
			{modes.protocol && <SimpleProtocol setModes={setModes} superMan={async p => (modes.protocol === 'punish' ? await superMan(p) : await linksHandler({ ...p, ...propss }))} obj={obj} target={obj.id} modes={modes} thisIs={'user'} brain={brain} nowAt={nowAt} setStatus={setStatus} chatObj={chatObj} chatID={chatObj?.id} />}
		</user-menu>
	);
};

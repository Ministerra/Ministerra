			...
			{!expanded ? (
				...
				<button onClick={() => setExpanded(true)} className="posRel padBotS     shaBlueLight borBotLight padBotXxs bgTrans boRadXxxs  zinMaXl bHover w100 mw90  ">
					...
					<content-wrapper class="posRel flexCol overHidden w100">
						<img src={`/icons/${currentKey === 'cityEvents' ? 'yourLoca' : 'topEvents'}.png`} alt="" className="bgTransXs marAuto aspect169    maskLowXs mw12 downTinyBit posRel" />
				...
				Object.entries(views).map(([key, { title, desc }]) => (
					<button key={key} onClick={() => (showMan('homeView', key), setExpanded(false))} style={{ width: '100%', ...(buttonWidth && { maxWidth: `${buttonWidth}px` }) }} className={`${show.view === key ? 'xBold' : ''} posRel bgTrans padTopS  padBotM boRadXxxs maskLowXs bHover grow`}>
						{/* SELECTION OPTION CONTENT --- */}
						{/* Displays visual and textual information for each view option */}
						<content-wrapper class="posRel flexCol overHidden w100">
							<img src={`/icons/${key === 'cityEvents' ? 'yourLoca' : 'topEvents'}.png`} alt="" className="bgTransXs   maskLowXs marAuto mw10 wvw12 posRel" />

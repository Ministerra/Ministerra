...
async function resolveDeviceSync(userID, devID, clientDevSync) {
	const numericClient = Number(clientDevSync) || 0;
	const now = Date.now();
	// CLIENT CLAMP ------------------------------------------------------------
	// Steps: cap future timestamps so a bad clock can’t “future-lock” the device forever.
	const safeClient = numericClient > now + 60000 ? now : numericClient;
	...
	try {
		...
		const stored = await redis.hget(userSummaryKey(userID), `devSync:${devID}`);
...
async function persistDeviceSync(userID, devID, devSync, linksSync) {
	...
	const latest = Math.max(Number(devSync) || 0, Number(linksSync) || 0);
	if (!latest) return;

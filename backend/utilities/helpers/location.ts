...
export interface LocationHashInput {
	city: string;
	part?: string | null;
	lat: number;
	lng: number;
...
export function generateLocationHash({ city, part, lat, lng }: LocationHashInput): string {
	// INPUT NORMALIZATION ---
...
export interface CityData extends LocationHashInput {
	cityID?: number;
	hashID?: string;
	county?: string | null;
	region?: string | null;
...
export async function getOrSaveCityData(con: PoolConnection | null, cities: (number | CityData)[]): Promise<CityData[]> {
	// REQUEST TRACE ------------------------------------------------------------
	...
	try {
		let existingCities: (CityData | null)[] = [],
			foundByhashIDs: (CityData | null)[] = [];

		// INPUT SPLIT ---
		// Steps: keep numeric cityIDs (already canonical) separate from objects (need resolution).

		const [cityIDsOnly, cityObjects] = cities.reduce<[number[], CityData[]]>(
			(acc, city) => {
				typeof city === 'object' ? acc[1].push(city as CityData) : acc[0].push(city as number);
				return acc;
			},
			[[], []]
		);
		...
		if (cityIDsOnly.length)
			existingCities = (await redis.hmgetBuffer(REDIS_KEYS.citiesData, ...cityIDsOnly.map(String)))
				.map((data, i) => (data ? ({ cityID: cityIDsOnly[i], ...(decode(data) as object) } as CityData) : null))
				.filter(Boolean);

		const validExistingCities = existingCities.filter(c => c !== null) as CityData[];
		...
		const hashIDs = cityObjects.map(c => c.hashID).filter((h): h is string => !!h);
		...
		if (hashIDs.length) {
			...
			if (existingCityIDs.length)
				foundByhashIDs = (await redis.hmgetBuffer(REDIS_KEYS.citiesData, ...existingCityIDs))
					.map((data, i) => (data ? ({ cityID: Number(existingCityIDs[i]), ...(decode(data) as object) } as CityData) : null))
					.filter(Boolean);
		}
		const validFoundByHashIDs = foundByhashIDs.filter((c): c is CityData => c !== null);
		const foundHashIDsSet = new Set(validFoundByHashIDs.map(c => c.hashID));

		// Combine our partial results
		const allFoundSoFar: CityData[] = [...validExistingCities, ...validFoundByHashIDs];
		...
		const newCitiesWithIDs: CityData[] = [];
		...
		if (newCities.length) {
			try {
				...
				const [newCityIDRows] = (await con.execute(/*sql*/ `SELECT id, hashID FROM cities WHERE hashID IN (${newhashIDs.map(() => '?').join(',')})`, newhashIDs)) as any;

				newCitiesWithIDs.push(
					...(newCities
						.map(city => {
							const matchingRow = newCityIDRows.find((row: any) => row.hashID === city.hashID);
							if (!matchingRow) {
